...
import { deliveryData, orderItems } from "../../components/data"
...
const ConfirmPayment = () => {
                ...
                <tr key={item.id} className="bg-white">
                  ...
                  <td className="p-1 text-center">
                    <div className="inline-flex items-center border border-gray-200 rounded-lg bg-gray-100">
                      <button className="p-2 border-r border-gray-200">
                        <DecrementIcon />
                      </button>
                      <span className="font-semibold min-w-[40px] text-center">
                        7
                      </span>
                      <button className="p-2 border-l border-gray-200">
                        <IncrementIcon />
                      </button>
                    </div>
                  </td>
  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    {/* Left Section - Summary of Charges */}
    <div>
     <div className="rounded-t-lg bg-gray-100 p-3">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">
          Summary of charges
...
<div className="my-10">
      <YellowButton text="Proceed to Next" wFull={true} handleClick={()=> setShowModal(true)} />
        <Modal
        show={showModal}
  ...
  return (
    <div className="my-4">
      ...
      <div className="space-y-4 p-4">
        {/* Map first 2 rows in pairs */}
        <div className="flex justify-between">
          {deliveryData.slice(0, 2).map((item, index) => (
            <div
              key={index}
              className={`flex-1 ${index === 0 ? "mr-8" : ""}`}
            >
        ...
        <div className="flex justify-between">
          {deliveryData.slice(2, 4).map((item, index) => (
            <div
              key={index}
              className={`flex-1 ${index === 0 ? "mr-8" : ""}`}
            >
        ...
        <div>
          <h3 className="text-sm font-semibold text-black mb-1">
            {deliveryData[4].label}
          </h3>
          <p className="text-sm text-[#83898F]">
            {deliveryData[4].value}
          </p>
        </div>
      </div>
    </div>
