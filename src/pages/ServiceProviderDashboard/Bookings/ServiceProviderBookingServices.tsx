...
const ServiceProviderBookingServices = ({ nextStep }: { nextStep: () => void }) => {
      const [searchQuery, setSearchQuery] = useState<string>("");
      const [openModalPart, setOpenModalPart] = useState<string | null>(null);
  ...
  <>
        ...
        <div className="rounded-t-lg bg-gray-100 p-2">
          <h3 className="text-lg font-semibold text-black mb-4">Select Services</h3>
        </div>
  ...
  <div>
          {filteredParts.map((part) => (
            <ServiceProductCard
              ...
              insideProducts={false}
               onOpenModal={(id) => setOpenModalPart(id)}
            />
          ))}
        </div>
        ...
        {openModalPart && (
  <BookingDateAndTime onClose={()=> setOpenModalPart('')} />
)}
      </>
  )
}

export default ServiceProviderBookingServices
